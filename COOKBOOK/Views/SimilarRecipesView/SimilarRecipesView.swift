...
class SimilarRecipesView: UIView {
    ...
    var similarRecipesVC = SimilarRecipesViewController()
...
extension SimilarRecipesView: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if section == 0 {
            ...
            return similarRecipesVC.recipeIngredients?.count ?? 0
        }
    }
...
//            
            ...
            cell.theNumberOfIngredient.text = "\(similarRecipesVC.sequence[indexPath.row])"
            cell.theNameOfIngredient.text = similarRecipesVC.recipeIngredients?[indexPath.row]
            if let weight = similarRecipesVC.ingredientsWeight?[indexPath.row] {
                cell.theAmountOfIngredient.text = "\(String(format: "%.2f", weight)) \(similarRecipesVC.ingredientsUnit?[indexPath.row] ?? "Error")"
            }
            return cell
        }
    }
